...
rm -fr $P $VT; mkdir $P
curl -s $VBU | textutil -stdin -stdout -convert txt > $VBT
cat $VBT | egrep -v '\\|’|\.$' | tr " " \\n | egrep '^[A-Z]' > $PT
for p in $(cat $PT); do
    p="${p:l}"; PPT="$P/$p.txt"; q=`urlenc -nt "[{ \"name\": null, \"name~=\": \"$p\" }]"`
    curl-s -L $WAU/$WRU/$p/ | egrep -io '"[^"]*'"(( ?$p)|($p ?))"'[^"]*"' >> $PPT
    curl -s $WZU$p | egrep -io '^(#|:)?\*[^\*{:]+[^{:]+' >> $PPT
    curl -s $TCU$p/ | egrep -io 'm>[^<>]{3,}</e' >> $PPT
    curl -s $FBU$q | egrep -io '.*name.*' >> $PPT
done
for f in $P/*; do
    echo ${f##*/} >> $VT
    cat $f >> $VT
